...
use super::{OperatorEntry, TimerKey, Workspace, TABLES_DIR};
    ...
    D: DispatchOperator,
{
    ...
    ///
    /// This is the legacy/local catalog view. Cluster desired state is modeled
    /// separately by `zendb_types::OperatorSpec` and the control-plane traits;
    /// this method does not imply that the current device should run it.
    ...
    ///
    /// This remains useful for embedded applications and tests, but it is a
    /// low-level compatibility API. A cluster-aware application should persist
    /// an `OperatorSpec` and let its reconciler decide whether to create a
    /// worker, acquire a lease, or remain stopped on this device.
    ...
    pub(crate) fn retire_operator(
        ...
        for (table_name, config) in tables {
            let result = if let Some(table) = self.tables.read().get(&table_name).cloned() {
                ...
                Table::open_with_device(&path, config, self.config.device_id)
