...
use crate::{ClusterConfig, TableConfig, Workspace, WorkspaceConfig};

use super::support::tmp;

type TestWorkspace = Workspace;
...
fn direct_bootstrap_requires_pre_admission_and_expected_peer_key() {
    ...
    let owner = TestWorkspace::create(&owner_path, config(&workspace_id, owner_id)).unwrap();
    ...
    owner
        .admit_device(
            ...
            DeviceRecord {
                name: "laptop".into(),
                key_ring: joiner.device(joiner_id).unwrap().unwrap().key_ring,
                roles: BTreeSet::new(),
...
fn cluster_runtime_reconnects_multiple_peers_and_delivers_departure() {
    ...
    loop {
        let converged = joiner
            ...
            .then(|| joiner.table("cluster-data").open().ok())
            .flatten()
            .and_then(|handle| handle.get().ok())
            .and_then(|table| {
                ReadBackend::get(&*table.read(), &PrimaryKey::String("row".into()))
...
fn ticket_bootstrap_installs_shared_snapshot_and_continues_with_anti_entropy() {
    ...
    let local_config = TableConfig::default();
    joiner
        .table("reader-local-data")
        .config(local_config)
        .create()
        .unwrap();
    let table = joiner.table("documents").open().unwrap().get().unwrap();
    assert_eq!(
        ReadBackend::get(&*table.read(), &PrimaryKey::String("one".into()))
            .unwrap()
            .value,
    ...
    let key = PrimaryKey::String("one".into());
    assert!(joiner.table("documents").row(key.clone()).local().unwrap());
    joiner
        .table("documents")
        .row(key.clone())
        .replace(Value::String("local-old".into()))
        .unwrap();
    ...
    owner
        .mutate(
            ...
            Path::new(),
            Op::Replace {
                value: Value::String("remote-newer".into()),
            },
        )
        .unwrap();
    ...
    joiner.sync_tcp(address).unwrap();
    server.join().unwrap();
    assert_eq!(
        joiner
            .table("documents")
            ...
            .unwrap()
            .value,
        Some(Value::String("local-old".into()))
    );
    ...
    assert_eq!(
        joiner
            ...
            .unwrap()
            .value,
        Some(Value::String("remote-newer".into()))
    );
}
