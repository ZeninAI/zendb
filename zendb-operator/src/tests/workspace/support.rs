...
use zendb_storage::backend::state::StateConfig;
use zendb_types::{
    device_id, init_device_id, Event, Hlc, Op, Path as ValuePath, PrimaryKey, Value, WorkspaceId,
};

type TestHost<D> = crate::OperatorHost<D>;
...
pub(crate) fn workspace_config() -> WorkspaceConfig {
    init_device_id();
    WorkspaceConfig {
...
// ---------------------------------------------------------------------------
// Workspace-specific test operators
// ---------------------------------------------------------------------------
...
pub(super) type TestWorkspace = TestHost<test_operators::OperatorInstance>;
