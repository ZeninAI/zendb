...
//! whose bincode byte representation has that ordering, or use a WriteBackend such
...
//! (`bulk_put`, `bulk_put_sorted`, `bulk_delete`) use WriteBackend-specific
...
// helpers. Every public method body lives in the `WriteBackend` / `OrderedReadBackend`
...
// ===========================================================================
// WriteBackend impl — all public method bodies live here.
// ===========================================================================
...
impl<K, V> ReadBackend<K, V> for BPlusTree<K, V>
where
    K: Encode + Decode<()> + Hash + Eq + Clone + Ord + Send + Sync + 'static,
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
        ...
        K: 'a,
    {
        RawBTreeKeys::new(self, self.first_leaf())
            .map(|key| Cow::Owned(deserialize_from(key).expect("valid encoded key")))
    }
        ...
        V: 'a,
    {
        RawBTreeEntries::new(self, self.first_leaf())
            .map(|(_, value)| Cow::Owned(deserialize_from(value).expect("valid encoded value")))
    }
...
impl<K, V> WriteBackend<K, V> for BPlusTree<K, V>
where
    K: Encode + Decode<()> + Hash + Eq + Clone + Ord + Send + Sync + 'static,
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
...
// ===========================================================================
// OrderedReadBackend impl
// ===========================================================================

impl<K, V> OrderedReadBackend<K, V> for BPlusTree<K, V>
where
    K: Encode + Decode<()> + Hash + Eq + Clone + Ord + Send + Sync + 'static,
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
...
mod tests {
    ...
    fn put_if_absent_inserts_when_missing() {
        ...
        let mut t = create(&p);
        assert!(WriteBackend::put_if_absent(&mut t, &k("a"), vbytes("first")).unwrap());
    ...
    fn put_if_absent_noop_when_present() {
        ...
        assert!(!WriteBackend::put_if_absent(&mut t, &k("a"), vbytes("second")).unwrap());
        assert_eq!(vget(&t, &k("a")), Some(vbytes("first")));
    }
    ...
    fn replace_returns_previous_value() {
        ...
        let mut t = create(&p);
        t.put(k("a"), vbytes("first")).unwrap();
        let prev = WriteBackend::replace(&mut t, &k("a"), vbytes("second"))
            .unwrap()
            .map(Cow::into_owned);
    ...
    fn replace_returns_none_when_absent() {
        ...
        let mut t = create(&p);
        let prev = WriteBackend::replace(&mut t, &k("a"), vbytes("new")).unwrap();
        assert!(prev.is_none());
    ...
    fn bulk_put_inserts_all_items() {
        ...
        let items: Vec<(TestKey, TestVal)> = (0u32..50)
            ...
            .collect();
        WriteBackend::bulk_put(&mut t, items).unwrap();
        assert_eq!(t.size(), 50);
    }
    ...
    fn bulk_put_sorted_single_entry() {
        ...
        let mut t = create(&p);
        WriteBackend::bulk_put_sorted(&mut t, vec![(k("only"), vbytes("v"))]).unwrap();
        assert_eq!(t.size(), 1);
    ...
    fn bulk_delete_returns_removed_count() {
        ...
        let n = WriteBackend::bulk_delete(&mut t, all).unwrap();
        assert_eq!(n, 5);
        assert_eq!(t.size(), 5);
    }
    ...
    fn bulk_delete_sorted_followed_by_put_does_not_corrupt() {
        ...
        let to_delete: Vec<TestKey> = (0u32..150)
            .map(|i| format!("k{:04}", i).into_bytes())
            .collect();
        WriteBackend::bulk_delete_sorted(&mut t, to_delete.iter()).unwrap();
    ...
    fn first_and_last_endpoints() {
        ...
        let (fk, _) = OrderedReadBackend::first(&t).unwrap();
        let (lk, _) = OrderedReadBackend::last(&t).unwrap();
    ...
    fn first_and_last_on_empty_tree() {
        ...
        let t = create(&p);
        assert!(OrderedReadBackend::first(&t).is_none());
        assert!(OrderedReadBackend::last(&t).is_none());
    }
    ...
    fn first_and_last_skip_empty_endpoint_leaves() {
        ...
        assert_eq!(t.size(), 100);
        let (fk, _) = OrderedReadBackend::first(&t).expect("first should find k0200");
        let (lk, _) = OrderedReadBackend::last(&t).expect("last should find k0299");
    ...
    fn entries_rev_yields_descending_order() {
        ...
        let keys: Vec<Vec<u8>> = OrderedReadBackend::entries_rev(&t)
            .map(|(k, _)| k.into_owned())
            .collect();
    ...
    fn range_rev_descending() {
        ...
        let got: Vec<Vec<u8>> = OrderedReadBackend::range_rev(&t, &k("k0005"), &k("k0010"))
            .map(|(k, _)| k.into_owned())
            .collect();
    ...
    fn range_rev_empty_when_start_eq_end() {
        ...
        let got: Vec<Vec<u8>> = OrderedReadBackend::range_rev(&t, &k("k0002"), &k("k0002"))
            .map(|(k, _)| k.into_owned())
            .collect();
        assert!(got.is_empty());
    }
    ...
    fn bulk_delete_sorted_frees_emptied_leaves() {
        ...
        let to_delete: Vec<TestKey> = (0u32..300)
            .map(|i| format!("k{:04}", i).into_bytes())
            .collect();
        let n = WriteBackend::bulk_delete_sorted(&mut t, to_delete.iter()).unwrap();
        assert_eq!(n, 300);
        assert_eq!(t.size(), 100);
    ...
    fn bulk_delete_sorted_deletes_all_entries() {
        ...
        let keys: Vec<TestKey> = (0u32..50)
            .map(|i| format!("k{:04}", i).into_bytes())
            .collect();
        let n = WriteBackend::bulk_delete_sorted(&mut t, keys.iter()).unwrap();
        assert_eq!(n, 50);
        assert_eq!(t.size(), 0);
        assert!(t.is_empty());
    ...
    fn range_rev_across_leaf_boundary() {
        ...
        let got: Vec<Vec<u8>> = OrderedReadBackend::range_rev(&t, &k("k0050"), &k("k0250"))
            .map(|(k, _)| k.into_owned())
            .collect();
