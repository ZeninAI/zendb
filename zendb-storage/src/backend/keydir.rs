...
//! every callable is exposed through the [`WriteBackend`](crate::backend::_traits::WriteBackend)
//! trait — there are no inherent `pub` accessors that duplicate it.
//! Callers should `use crate::backend::_traits::WriteBackend;` and reach the
...
/// it comes straight from `index.len()` via `ReadBackend::size`, so there's
...
// so WriteBackend methods that hold a HashMap `Entry` / `RawEntryMut` open
    ...
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
    ...
    // detail. The WriteBackend trait impl at the bottom of the file is what
    ...
    /// dead-byte ratio is over it, calls the [`WriteBackend::compact`]
...
impl<K, V> Drop for KeyDir<K, V> {
    ...
    /// [`crate::backend::Barrier::Sync`] explicitly before dropping.
    fn drop(&mut self) {
        let _ = KeyDir::persist(self, crate::backend::Barrier::Flush);
    }
}
...
// ---------------------------------------------------------------------------
// WriteBackend impl — the entirety of KeyDir's public surface besides
...
impl<K, V> crate::backend::_traits::ReadBackend<K, V> for KeyDir<K, V>
where
    K: Encode + Decode<()> + Hash + Eq + Clone + Ord + Send + Sync + 'static,
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
...
impl<K, V> crate::backend::_traits::WriteBackend<K, V> for KeyDir<K, V>
where
    K: Encode + Decode<()> + Hash + Eq + Clone + Ord + Send + Sync + 'static,
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
...
mod tests {
    use super::*;
    use crate::backend::_traits::{ReadBackend, WriteBackend};
    use std::borrow::Cow;
    use std::fs;
    use std::path::PathBuf;
    use std::sync::atomic::{AtomicU64, Ordering};

    /// Test helper — read through the Cow-returning ReadBackend::get API
    ...
    where
        ...
        V: Encode + Decode<()> + Clone + Send + Sync + 'static,
    {
        ReadBackend::get(kd, key).map(Cow::into_owned)
    }
    ...
    fn reopen_preserves_live_entries() {
        let p = tmp_path("persist_live");
        {
            ...
            kd.put(k("a"), v("alpha", 1)).unwrap();
            kd.put(k("b"), v("beta", 2)).unwrap();
            kd.persist(crate::backend::Barrier::Flush).unwrap();
        }
        let kd = open(&p);
        assert_eq!(kd.size(), 2);
        assert_eq!(get(&kd, &k("a")), Some(v("alpha", 1)));
    ...
    fn zero_ratio_compacts_on_first_write_after_reopen() {
        let p = tmp_path("compact_zero_after_reopen");
        {
            ...
            assert!(kd.stats().dead_bytes > 0);
            kd.persist(crate::backend::Barrier::Sync).unwrap();
        }
    ...
    fn put_if_absent_inserts_when_missing() {
        ...
        let mut kd = create(&p);
        let inserted = WriteBackend::put_if_absent(&mut kd, &k("a"), v("first", 1)).unwrap();
        assert!(inserted);
    ...
    fn put_if_absent_noop_when_present() {
        ...
        let mut kd = create(&p);
        kd.put(k("a"), v("first", 1)).unwrap();
        let inserted = WriteBackend::put_if_absent(&mut kd, &k("a"), v("second", 2)).unwrap();
        assert!(!inserted);
        assert_eq!(get(&kd, &k("a")), Some(v("first", 1)));
    }
    ...
    fn replace_returns_previous_value() {
        ...
        let mut kd = create(&p);
        kd.put(k("a"), v("first", 1)).unwrap();
        let prev = WriteBackend::replace(&mut kd, &k("a"), v("second", 2))
            .unwrap()
            .map(Cow::into_owned);
    ...
    fn replace_returns_none_when_absent() {
        ...
        let mut kd = create(&p);
        let prev = WriteBackend::replace(&mut kd, &k("a"), v("new", 99)).unwrap();
        assert!(prev.is_none());
    ...
    fn bulk_put_inserts_all_items() {
        ...
        let items: Vec<(TestKey, TestVal)> = (0..50u32)
            ...
            .collect();
        WriteBackend::bulk_put(&mut kd, items).unwrap();
    ...
    fn bulk_delete_returns_removed_count() {
        ...
        let n = WriteBackend::bulk_delete(&mut kd, all).unwrap();
        assert_eq!(n, 5);
        assert_eq!(kd.size(), 5);
    }
    ...
    fn replace_inserts_when_absent_and_persists() {
        // The WriteBackend override of `replace` exercises the Vacant->Insert
    ...
    fn replace_returns_old_value_and_accounts_dead_bytes() {
        ...
        kd.put(k("a"), v("v1", 1)).unwrap();
        assert_eq!(kd.stats().dead_bytes, 0);
        let prev = WriteBackend::replace(&mut kd, &k("a"), v("v2", 2))
            .unwrap()
            .map(Cow::into_owned);
    ...
    fn into_owned_yields_decoded_values() {
        ...
        let owned: TestVal = ReadBackend::get(&kd, &k("a")).unwrap().into_owned();
        assert_eq!(owned, v("alpha", 1));

        let owned_entries: Vec<(TestKey, TestVal)> = ReadBackend::entries(&kd)
    ...
    fn zero_byte_value_round_trips() {
        ...
        assert_eq!(kd.size(), 2);
        assert_eq!(
            ReadBackend::get(&kd, &k("alpha")).map(Cow::into_owned),
            Some(())
        );
        assert!(ReadBackend::get(&kd, &k("beta")).is_none());
        assert_eq!(
            ReadBackend::get(&kd, &k("gamma")).map(Cow::into_owned),
            Some(())
        );
    }
}
