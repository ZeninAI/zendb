//! Entirely in-memory arena-backed skip-list WriteBackend.
...
/// An entirely in-memory ordered WriteBackend. Construct it with [`SkipList::new`].
///
...
impl<K, V> ReadBackend<K, V> for SkipList<K, V>
where
    K: Encode + Decode<()> + Hash + Eq + Clone + Ord + Send + Sync + 'static,
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
...
impl<K, V> WriteBackend<K, V> for SkipList<K, V>
where
    K: Encode + Decode<()> + Hash + Eq + Clone + Ord + Send + Sync + 'static,
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
...
impl<K, V> OrderedReadBackend<K, V> for SkipList<K, V>
where
    K: Encode + Decode<()> + Hash + Eq + Clone + Ord + Send + Sync + 'static,
    V: Encode + Decode<()> + Clone + Send + Sync + 'static,
{
...
mod tests {
    ...
    fn bulk_put_sorted_merges_and_overwrites_in_order() {
        ...
        WriteBackend::bulk_put_sorted(&mut list, [(2, 20), (3, 300), (3, 333), (4, 40), (6, 60)])
            .unwrap();
    ...
    fn bulk_delete_sorted_sweeps_hits_and_misses() {
        ...
        let keys = [0, 2, 4, 4, 7];
        let removed = WriteBackend::bulk_delete_sorted(&mut list, keys.iter()).unwrap();
