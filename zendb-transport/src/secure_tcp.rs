//! Mutually authenticated, forward-secret TCP sessions.
...
/// above this type; plaintext never reaches the underlying TCP stream after
...
impl SecureTcpSession {
    pub fn connect(
        ...
        Self::connect_stream(stream, workspace_id, profile, purpose, authorize_peer)
    }
    ...
    pub fn connect_stream(
        ...
        write_plain(&mut stream, &local)?;
        let remote: SecureHello = read_plain(&mut stream, 64 * 1024)?;
        ...
        write_plain(&mut stream, &local_proof)?;
        let remote_proof: SecureProof = read_plain(&mut stream, 64 * 1024)?;
        ...
        Ok(Self::new(stream, peer, keys.0, keys.1))
    }
    ...
    pub fn accept(
        ...
        let keys = derive_keys(secret, remote.ephemeral_key, &transcript)?;
        Ok(Self::new(stream, peer, keys.1, keys.0))
    }
    ...
    pub fn send(&mut self, plaintext: &[u8]) -> io::Result<()> {
        ...
        write_bytes(&mut self.stream, &encrypted)
    }

    pub fn receive(&mut self) -> io::Result<Vec<u8>> {
        let encrypted = read_bytes(&mut self.stream, self.max_frame_bytes + 16)?;
...
fn write_plain<T: Encode>(stream: &mut TcpStream, value: &T) -> io::Result<()> {
    let bytes = bincode::encode_to_vec(value, bincode::config::standard())
        .map_err(|error| io::Error::other(error.to_string()))?;
    write_bytes(stream, &bytes)
}

fn read_plain<T: Decode<()>>(stream: &mut TcpStream, max: usize) -> io::Result<T> {
    let bytes = read_bytes(stream, max)?;
    let (value, consumed) = bincode::decode_from_slice(&bytes, bincode::config::standard())
        .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error.to_string()))?;
    if consumed != bytes.len() {
        return Err(io::Error::new(
...
fn read_bytes(stream: &mut TcpStream, max: usize) -> io::Result<Vec<u8>> {
    let mut length = [0; 4];
...
mod tests {
    ...
    fn encrypted_session_authenticates_and_exchanges_messages() {
        ...
        let workspace = WorkspaceId::from("secure-test");
        ...
        let worker = thread::spawn(move || {
            ...
            let mut session =
                SecureTcpSession::accept(stream, &server_workspace, &server_profile, |peer| {
