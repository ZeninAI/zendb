//! Device registry, installation clocks, receipts, and authorization.
...
use super::{
    device_primary_key, installation_id_from_key,
    receipts::{ObserveOutcome, ReceiptWindow},
};
...
impl Devices {
    pub(crate) fn create(
        ...
        registry.insert(Event {
            primary_key: device_primary_key(local_installation_id),
            path: Path::new(),
            op: Op::Upsert {
    ...
    pub(crate) fn open(
        ...
        for (key, cell) in registry.entries() {
            ...
            let Some(installation_id) = installation_id_from_key(&key) else {
                return Err(Error::CorruptDeviceRegistry(
                    "devices table contains a non-installation key".to_owned(),
                ));
            };
    ...
    pub fn upsert(&self, installation_id: InstallationId, record: DeviceRecord) -> Result<bool> {
        ...
        self.registry.insert_internal(Event {
            primary_key: device_primary_key(installation_id),
            path: Path::new(),
            op: Op::Upsert {
                value: Value::Blob(Blob::encode(&record)?),
            },
