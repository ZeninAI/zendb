...
//! and produces Graft/Prune actions to converge toward `target_neighbors`.

use std::collections::{HashMap, HashSet, hash_map::DefaultHasher};
use std::hash::{Hash, Hasher};

use libp2p::{Multiaddr, PeerId};
use zendb_types::Installation;
...
impl Mesh {
    ...
    /// Register or update an active installation route.
    pub(super) fn add(&mut self, installation: &Installation) {
    ...
    /// Remove an installation route that is no longer active.
    pub(super) fn remove(&mut self, installation: &Installation) {
        let peer_id = installation.public_key.as_libp2p().to_peer_id();
    ...
    pub(super) fn maintain(&mut self) -> Vec<MeshAction> {
        let mut candidates: Vec<(PeerId, u64)> = self
            ...
            .map(|(peer_id, _)| {
                (
                    *peer_id,
                    mesh_rank(self.local_peer_id, *peer_id, self.epoch),
                )
            })
            .collect();
        candidates.sort_unstable_by_key(|(_, rank)| *rank);

        let target = self.config.target_neighbors.min(candidates.len());
    ...
    pub(super) fn accepts(&self, peer_id: PeerId) -> bool {
        self.routes.get(&peer_id).is_some_and(|r| r.active)
            && (self.peers.len() < self.config.max_neighbors || self.peers.contains(&peer_id))
    }
